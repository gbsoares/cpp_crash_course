...
/* function declaration for output stream operator */
std::ostream& operator<<(std::ostream& s, const AutoBrake& auto_brake);
...
double AutoBrake::get_speed_limit() const
{
    return speed_limit_mps;
}

/* Output operator for custom user-type */
std::ostream& operator<<(std::ostream& s, const AutoBrake& auto_brake)
{
    s << "AutoBrake info:\n";
    s << " - Collision Thresh: " << auto_brake.get_collision_threshold_s() << "\n";
    s << " - Speed (mps): " << auto_brake.get_speed_mps() << "\n"; 
    return s;
}
